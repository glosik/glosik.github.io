...
function contact() {
    ...
    form.submit(function (e) {
        ...
        $.post(
            action, //A string containing the URL to which the request is sent.
            $this.serialize(),
            function (data) { //A callback that is executed if the request succeeds
                //$this.prev().text(data.message).fadeIn().delay(3000).fadeOut();
                $this.prev().text(data).fadeIn().delay(3000).fadeOut();
                //form.hide(); //hide form once sent
            }, 'json'); //data type
//        return false;
    });
}
